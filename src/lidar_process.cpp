...
int main(int argc, char** argv) {
    ...
    crash_sub = nh.subscribe("/auto_car/crash", 100, &crashCallback);
    // Création des publishers -----------------------------------------------------------------------------------
    processed_lidar = nh.advertise<auto_car_ctrl::rosFloat>("auto_car/lidar_process", 100);
    ...
    if(!crash) {
        while(delAngle.size() < scan_msg->ranges.size()) {
            ...
            for(int i = 0; i <= scan_msg->ranges.size(); i++) {
                ...
                if((left_edge_vision > angle) && (angle > right_edge_vision) && (scan_msg->ranges[i] < accept_range) && (scan_msg->ranges[i] > car_size)) {
